...
var SQLiteAdapter = /*#__PURE__*/function () {
  ...
  _proto.testClone = function testClone(options = {}) {
    return new Promise(function ($return, $error) {
      ...
      return Promise.resolve(clone._initPromise).then(function ($await_1) {
        try {
          return $return(clone);
        } catch ($boundEx) {
  ...
  _proto.unsafeExecute = function unsafeExecute(operations, callback) {
    if (process.env.NODE_ENV !== 'production') {
      (0, _common.invariant)(operations && typeof operations === 'object' && Object.keys(operations).length === 1 && (Array.isArray(operations.sqls) || typeof operations.sqlString === 'string'), "unsafeExecute expects an { sqls: [ [sql, [args..]], ... ] } or { sqlString: 'foo; bar' } object");
    }
    if (operations.sqls) {
