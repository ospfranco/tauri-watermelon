...
var unsafeFetchAsRaws = function unsafeFetchAsRaws(query) {
...
var destroyAllDeletedRecords = function destroyAllDeletedRecords(db, recordsToApply) {
  return new Promise(function ($return, $error) {
    var promises;
    promises = (0, _fp.toPairs)(recordsToApply).map(function ([tableName, {
...
var applyAllRemoteChanges = function applyAllRemoteChanges(recordsToApply, context) {
  return new Promise(function ($return, $error) {
    var db, allRecords;
    ({
      db: db
    ...
    // $FlowFixMe
    return Promise.resolve(db.batch(allRecords)).then(function ($await_8) {
      try {
        return $return();
      } catch ($boundEx) {
        return $error($boundEx);
...
var unsafeApplyAllRemoteChangesByBatches = function unsafeApplyAllRemoteChangesByBatches(recordsToApply, context) {
  return new Promise(function ($return, $error) {
    var db, promises;
    ({
      db: db
    ...
    return Promise.resolve(Promise.all(promises)).then(function ($await_9) {
      try {
        return $return();
      } catch ($boundEx) {
        return $error($boundEx);
  ...
  return new Promise(function ($return, $error) {
    ...
    return Promise.resolve(getAllRecordsToApply(remoteChanges, context)).then(function ($await_10) {
      try {
        ...
        return Promise.resolve(Promise.all([destroyAllDeletedRecords(db, recordsToApply), _unsafeBatchPerCollection ? unsafeApplyAllRemoteChangesByBatches(recordsToApply, context) : applyAllRemoteChanges(recordsToApply, context)])).then(function ($await_11) {
          try {
            return $return();
          } catch ($boundEx) {
            return $error($boundEx);
