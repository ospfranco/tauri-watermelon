  ...
  return new Promise(function ($return, $error) {
    ...
    return Promise.resolve(getLastPulledAt(database)).then(function ($await_2) {
      try {
        ...
        return Promise.resolve(database.adapter.setLocal(lastPulledAtKey, "".concat(timestamp))).then(function ($await_3) {
          try {
            return $return();
          } catch ($boundEx) {
            return $error($boundEx);
...
function setLastPulledSchemaVersion(database, version) {
  return new Promise(function ($return, $error) {
    return Promise.resolve(database.adapter.setLocal(lastPulledSchemaVersionKey, "".concat(version))).then(function ($await_5) {
      try {
        return $return();
      } catch ($boundEx) {
        return $error($boundEx);
