...
function markLocalChangesAsSynced(db, syncedLocalChanges, rejectedIds) {
  return db.write(function () {
    return new Promise(function ($return, $error) {
      return Promise.resolve(Promise.all([db.batch(recordsToMarkAsSynced(syncedLocalChanges, rejectedIds || {}).map(_helpers.prepareMarkAsSynced))].concat((0, _toConsumableArray2.default)(destroyDeletedRecords(db, syncedLocalChanges, rejectedIds || {}))))).then(function ($await_1) {
        try {
          return $return();
        } catch ($boundEx) {
